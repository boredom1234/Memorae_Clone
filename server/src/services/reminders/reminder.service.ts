...
import { Reminder, ReminderCreationParams, ReminderUpdateParams } from "./types";
import {
  validate,
...
export class ReminderService {
  ...
  async updateReminder(params: ReminderUpdateParams): Promise<{ success: boolean; message: string; updatedReminder: Reminder }> {
    const startTime = Date.now();
    try {
      ...
      if (validatedParams.reminderTime) updateData.reminder_time = toUTC(validatedParams.reminderTime);
  ...
  async deleteReminder(params: { userId: string; reminderId?: string; searchQuery?: string }): Promise<{ success: boolean; deletedCount: number; message: string }> {
    const startTime = Date.now();
    try {
      ...
      logAudit("DELETE_REMINDER", validatedParams.userId, "reminder", { deletedCount, reminderId: validatedParams.reminderId, searchQuery: validatedParams.searchQuery });
  ...
  async batchCreateReminders(params: { userId: string; reminders: Array<{ title: string; reminderTime: string; isRecurring?: boolean; recurrenceRule?: string }> }): Promise<{ success: boolean; created: number; failed: number; results: Array<{ title: string; success: boolean; reminderId?: string; error?: string }> }> {
    const startTime = Date.now();
    try {
      ...
      for (const reminder of validatedParams.reminders) {
        try {
          ...
          results.push({ title: reminder.title, success: true, reminderId: result.reminderId });
          created++;
        } catch (error: any) {
          logError("Failed to create reminder in batch", error, { title: reminder.title });
          results.push({ title: reminder.title, success: false, error: error.message || "Unknown error" });
          failed++;
        }
      }
  ...
  async archiveReminder(params: { userId: string; reminderId: string }): Promise<{ success: boolean; message: string }> {
    const startTime = Date.now();
    try {
      ...
      logAudit("ARCHIVE_REMINDER", existing.user_id, "reminder", { reminderId: validatedParams.reminderId, title: existing.title });
      ...
      logError("Failed to archive reminder", error, { reminderId: params.reminderId });
      throw handleServiceError(error, "archiveReminder");
    }
  }

  async cancelReminder(params: { userId: string; reminderId: string }): Promise<{ success: boolean; message: string }> {
    const startTime = Date.now();
    try {
  ...
  async rescheduleReminder(params: { userId: string; reminderId: string; newReminderTime: string }): Promise<{ success: boolean; message: string; newTime: string }> {
    const startTime = Date.now();
    try {
      ...
      const { data, error } = await this.supabase
        .from("reminders")
        .update({ reminder_time: toUTC(newReminderTime), status: "pending", updated_at: new Date().toISOString() })
        .eq("id", reminderId)
        .eq("user_id", userId)
        .select()
        .single();
