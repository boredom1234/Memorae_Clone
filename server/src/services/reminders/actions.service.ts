...
export class ReminderActionsService {
  ...
  async snoozeReminder(params: { userId: string; reminderId: string; snoozeUntil: string; snoozeDuration?: number }): Promise<{ success: boolean; newReminderTime: string; message: string }> {
  ...
  async completeReminder(params: { userId: string; reminderId: string }): Promise<{ success: boolean; message: string }> {
    const startTime = Date.now();
    try {
  ...
  async snoozeReminderByText(params: { userId: string; reminderId: string; text: string; timezone: string }): Promise<{ success: boolean; newReminderTime: string; message: string }> {
    try {
      const parsed = this.util.parseNaturalLanguageDate({ text: params.text, timezone: params.timezone });
      ...
      return await this.snoozeReminder({ userId: params.userId, reminderId: params.reminderId, snoozeUntil: ensured });
    } catch (error) {
  ...
  async bulkUpdateStatus(params: { userId: string; reminderIds: string[]; status: "completed" | "cancelled" | "pending" }): Promise<{ success: boolean; updated: number }> {
    try {
