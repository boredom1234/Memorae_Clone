...
export class ReminderQueryService {
  ...
  async listReminders(params: { userId: string; status?: "pending" | "completed" | "all"; startDate?: string; endDate?: string; limit?: number; offset?: number; sortBy?: "time" | "priority" | "created" }): Promise<{ reminders: Array<{ id: string; title: string; reminderTime: string; isRecurring: boolean; priority: string; createdAt: string }>; total: number; hasMore: boolean }> {
  ...
  async getUpcomingReminders(params: { userId: string; timeframe: "today" | "tomorrow" | "week" | "month"; limit?: number }): Promise<{ reminders: UpcomingReminder[]; total: number }> {
    const startTime = Date.now();
    try {
  ...
  async searchReminders(params: { userId: string; query: string; filters?: { dateRange?: { start: string; end: string }; priority?: "low" | "medium" | "high"; status?: "pending" | "completed" }; limit?: number }): Promise<{ results: SearchResult[]; total: number }> {
    const startTime = Date.now();
    try {
  ...
  async listOverdueReminders(params: { userId: string; limit?: number; offset?: number }): Promise<{ reminders: any[]; total: number }> {
    try {
