...
import { validate, parseNaturalLanguageDateSchema, detectIntentSchema, suggestReminderTimeSchema } from '../utils/validators';
...
export class UtilityService {
  parseNaturalLanguageDate(params: {
    ...
    try {
      ...
      const referenceDate = validatedParams.referenceDate ? new Date(validatedParams.referenceDate) : new Date();
      ...
      logPerformance('parseNaturalLanguageDate', Date.now() - startTime, { count: extractedDates.length });
      ...
      logError('Failed to parse natural language date', error, { text: params.text });
  ...
  detectIntent(params: {
    ...
    try {
      ...
      if (!validatedParams.message || validatedParams.message.trim().length === 0) {
          ...
          entities.dates = dateResults
            .map((r) => {
              try {
                ...
                logWarn('Failed to parse date in intent detection', { text: r.text });
                return null;
              }
            })
      ...
      logPerformance('detectIntent', Date.now() - startTime, { intent, confidence });
  ...
  suggestReminderTime(params: {
    ...
    try {
      ...
      if (!validatedParams.taskDescription || validatedParams.taskDescription.trim().length === 0) {
      ...
      if (validatedParams.userSchedule && validatedParams.userSchedule.length > 0) {
        logWarn('User schedule provided but smart scheduling not yet implemented');
      }

      logPerformance('suggestReminderTime', Date.now() - startTime, { count: suggestedTimes.length });

      return { suggestedTimes };
    } catch (error) {
      logError('Failed to suggest reminder time', error, { taskDescription: params.taskDescription });
