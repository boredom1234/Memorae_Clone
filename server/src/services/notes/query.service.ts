...
import { UserNote, SearchNotesParams, ListNotesParams, NoteWithMedia } from "./types";
...
export class NotesQueryService {
  ...
  async searchNotes(params: SearchNotesParams): Promise<{ notes: UserNote[]; total: number }> {
    try {
      ...
      const notes = paged.map((note: any) => this.notesService.mapDatabaseNote(note));

      logInfo("Notes search completed", { userId: params.userId, resultCount: notes.length, total: totalFull });
  ...
  async listNotes(params: ListNotesParams): Promise<{ notes: UserNote[]; total: number }> {
    try {
      ...
      const notes = data?.map((note: any) => this.notesService.mapDatabaseNote(note)) || [];
      logInfo("Notes listed successfully", { userId: params.userId, resultCount: notes.length, total: count || 0 });
  ...
  async getCategories(userId: string): Promise<string[]> {
    try {
      ...
      const categories = data
        ? ([...new Set(data.map((item: any) => item.category).filter(Boolean))] as string[])
        : [];
      return categories;
    } catch (error) {
  ...
  async getNotesStats(userId: string): Promise<{ total: number; categories: { [key: string]: number }; pinned: number; recent: number }> {
    try {
      const supabase = getSupabaseClient();
      ...
      throw error instanceof AppError
        ? error
        : new AppError("Failed to get notes stats", 500, "GET_NOTES_STATS_ERROR");
    }
  }

  async getNotesWithMedia(userId: string, options?: { category?: string; tags?: string[]; includeArchived?: boolean; limit?: number; offset?: number }): Promise<{ notes: NoteWithMedia[]; total: number }> {
    try {
      ...
      if (error) {
        ...
        throw new AppError("Failed to get notes with media", 500, "GET_NOTES_WITH_MEDIA_ERROR");
      }
      ...
      logInfo("Notes with media retrieved", { userId, resultCount: notesWithMedia.length, total: count || 0 });
      ...
      throw error instanceof AppError
        ? error
        : new AppError("Failed to get notes with media", 500, "GET_NOTES_WITH_MEDIA_ERROR");
    }
  }
}
